...
public class RewardManager : MonoBehaviour
{
    ...
    {
        // --- [범인 색출용 안전장치] 누가 없는지 검사합니다 ---
        if (InventoryManager.Instance == null) { Debug.LogError("🚨 InventoryManager가 씬에 없습니다! GameManager에 붙어있는지 확인하세요."); return; }
        if (GridManager.Instance == null) { Debug.LogError("🚨 GridManager가 씬에 없습니다!"); return; }
        if (MonsterManager.Instance == null) { Debug.LogError("🚨 MonsterManager가 씬에 없습니다!"); return; }
        if (HandManager.Instance == null) { Debug.LogError("🚨 HandManager에 Instance 설정이 안 되어 있습니다! Awake 함수를 확인하세요."); return; }
        if (PlayerManager.Instance == null) { Debug.LogError("🚨 PlayerManager가 씬에 없습니다!"); return; }
        ...
        // [이곳에 추가!] 보상 획득이 끝났으니, 상태를 저장하고 맵으로 귀환합니다.
